...
public class Producer
{
    ...
    public Producer( int id, ConcurrentQueue<int> sharedQueue)
    {
        this.producerId = id;
        this.random = new Random(id * 1000); // Verschiedene Seeds für verschiedene Producer
        
        // Thread im Konstruktor starten
        producerThread = new Thread(() => ProduceNumbers(sharedQueue));
        producerThread.Start();
    }

    private void ProduceNumbers(ConcurrentQueue<int> sharedQueue)
    {
        while (!shouldStop)
        {
